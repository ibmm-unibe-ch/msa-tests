...
function af_cluster() {
	...
	python AF-cluster.py $1 -i $2 -o $3
}
...
for a in "${array[@]}"; do
	echo $a |
	 ...
	 do
		...
		python get_secstrucs.py --input_pdb $FULL_A --output_json ${FULL_A%?????}_sec_struc.json --alignment_path $ALIGNMENT_A --cut_start $OFFSET_A --cut_length $LENGTH_A
		...
		python get_secstrucs.py --input_pdb $FULL_B --output_json ${FULL_B%????}_sec_struc.json --alignment_path $ALIGNMENT_B --cut_start $OFFSET_B --cut_length $LENGTH_B

		python get_secstrucs.py --input_pdb $PROT_MPNN_CONF_A_RF --output_json ${PROT_MPNN_CONF_A_RF%????}_sec_struc.json
		python get_secstrucs.py --input_pdb $PROT_MPNN_CONF_B_RF --output_json ${PROT_MPNN_CONF_B_RF%????}_sec_struc.json
	 	# AF2, Natural models
		for FOUND_FILE in $ALPHAFOLD_A/*rank_00[1-5]*.pdb; do
			...
			score $ALPHAFOLD_CONF_A/$FOUND_FILENAME $FULL_A $AF2_SCORE_DIR_A/${FOUND_FILENAME%????}_score.json
 			python get_secstrucs.py --input_pdb $ALPHAFOLD_CONF_A/$FOUND_FILENAME --output_json $AF2_SCORE_DIR_A/${FOUND_FILENAME%????}_sec_struc.json
		done
		...
		for FOUND_FILE in $PROT_MPNN_A/*rank_00[1-5]*.pdb; do
			...
			score $PROT_MPNN_CONF_A_DIR/$FOUND_FILENAME $FULL_A $SCORE_APROT_DIR/${FOUND_FILENAME%????}_score.json
 			python get_secstrucs.py --input_pdb $PROT_MPNN_CONF_A_DIR/$FOUND_FILENAME --output_json $SCORE_APROT_DIR/${FOUND_FILENAME%????}_sec_struc.json
	 	done
		for FOUND_FILE in $PROT_MPNN_B/*rank_00[1-5]*.pdb; do
			...
			score $PROT_MPNN_CONF_B_DIR/$FOUND_FILENAME $FULL_B $SCORE_BPROT_DIR/${FOUND_FILENAME%????}_score.json
 			python get_secstrucs.py --input_pdb $PROT_MPNN_CONF_B_DIR/$FOUND_FILENAME --output_json $SCORE_BPROT_DIR/${FOUND_FILENAME%????}_sec_struc.json
		done
		...
		for number in "${numbers[@]}"; do
			...
			python get_secstrucs.py --input_pdb $FOUND_FILE --output_json ${SCORE_APROT_DIR_AF3}_sec_struc/${FOUND_FILENAME%????}_sec_struc.json
		done
		for number in "${numbers[@]}"; do
			...
			python get_secstrucs.py --input_pdb $FOUND_FILE --output_json ${SCORE_BPROT_DIR_AF3}_sec_struc/${FOUND_FILENAME%????}_sec_struc.json
		done
		#Filter out similar sequences
		echo $ID_A
		python filter_similar.py --input $PROT_MPNN_A3M_A --output $DIFF_SEQ_A3M_A --seqid $SEQ_IDENTITY 
		python filter_similar.py --input $PROT_MPNN_A3M_B --output $DIFF_SEQ_A3M_B --seqid $SEQ_IDENTITY 
