...
function carbon() {
	pdb_tofasta $1>$2
	micromamba run -n carbonara python utils/carbonar.py --num_sequences 127 --imprint_ratio 0.5 $1 $2
}
...
function relax(){
	...
	micromamba run -n relaxation python utils/amber_relax.py --input ${1}_relaxed.pdb --output ${1}_relaxed.pdb
}
