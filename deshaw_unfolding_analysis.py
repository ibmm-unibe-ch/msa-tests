...
from md_traj_utils import compute_PCA, unpickle_obj, pickle_obj, make_deshaw_plot, clean_traj_c_alpha, compute_all_rmsds, best_hummer_q
...
OTHER_COLOUR = "gray" #"Mutations":[23,26,28]
...
def make_long_plot_q_colour(simulation, all_rmsds, ref, decision_boundary, save_path, pred_traj, simulation_sampling=SIMULATION_SAMPLING):
    ...
    for ax in [ax2]: #ax1
        for pred_x_value, missing_neighbour in zip(pred_x_values, missing_neighbours):
            if missing_neighbour < FRAMES_TO_EXCLUDE:
...
def make_sns_plot(point_list, plt_title,num_predictions=1, save_path=None, variance=None):
    ...
    sns.kdeplot(data=pca, x=f"PC1", y=f"PC2", palette=["gray"],color="gray", legend="Simulation", fill=True, alpha=0.8, thresh=0.001, levels=20)
    if variance is None:
        plt.xlabel(f"PC1")
        plt.ylabel(f"PC2")
    else:
...
if __name__ == "__main__":
    for protein_name, protein_info in INTERESTING_DESHAW_PROTEINS.items():
        ...
        make_sns_plot(comb_pca_coords,"Combined PCA",len(clean_pred_traj), save_path=protein_stub/"combined_pca_thresh_001.pdf", variance=comb_pca.explained_variance_ratio_)
